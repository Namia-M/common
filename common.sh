...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/Namia-M/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
sed -i '/Namia-M/d; /helloworld/d; /passwall/d; /OpenClash/d' "feeds.conf.default"
cat feeds.conf.default|awk '!/^#/'|awk '!/^$/'|awk '!a[$1" "$2]++{print}' >uniq.conf
mv -f uniq.conf feeds.conf.default

# 这里增加了源,要对应的删除/etc/opkg/distfeeds.conf插件源
cat >>"feeds.conf.default" <<-EOF
src-git danshui1 https://github.com/kenzok8/jell



src-git namia https://github.com/Namia-M/package
...
if [[ -f "${HOME_PATH}/feeds/luci/modules/luci-mod-system/root/usr/share/luci/menu.d/luci-mod-system.json" ]]; then
  echo "src-git danshui2 https://github.com/Namia-M/openwrt-package.git;Theme2" >> "feeds.conf.default"
else
  echo "src-git danshui2 https://github.com/Namia-M/openwrt-package.git;Theme1" >> "feeds.conf.default"
fi
...
if [[ "${REPO_BRANCH}" =~ (19.07|21.02|22.03) ]]; then
  if [[ -d "${HOME_PATH}/feeds/passwall3/shadowsocksr-libev" ]]; then
    curl -o ${HOME_PATH}/feeds/passwall3/shadowsocksr-libev/Makefile https://raw.githubusercontent.com/Namia-M/common/main/Share/shadowsocksr-libev/Makefile
  fi
  # 降低shadowsocks-rust版本,最新版本编译不成功
  if [[ -d "${HOME_PATH}/feeds/passwall3/shadowsocks-rust" ]]; then
    curl -o ${HOME_PATH}/feeds/passwall3/shadowsocks-rust/Makefile https://raw.githubusercontent.com/Namia-M/common/main/Share/shadowsocks-rust/Makefile
  fi
fi
}
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/Namia-M/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/Namia-M/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/Namia-M/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
